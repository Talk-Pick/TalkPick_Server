...
public interface RandomCommandApi {
    ...
    void start(@MemberId final Long memberId);

    @PostMapping("/{id}/next")
    @Operation(summary = "랜덤 대화 주제 코스 다음 API", description = "랜덤 대화 주제 코스 다음 API 입니다. 랜덤 대화 주제 코스에서 다음으로 넘어갈 때마다, 해당 API를 한 번 요청해 주세요.")
    void next(@MemberId final Long memberId, @PathVariable("id") @NotNull(message = "[ERROR] id 값이 존재하지 않습니다.") final Long randomId, @RequestBody RandomReqDTO.Next requestDTO);

    @PostMapping("/{id}/quit")
    @Operation(summary = "랜덤 대화 주제 코스 그만하기 API", description = "랜덤 대화 주제 코스 그만하기 API 입니다. 랜덤 대화 주제 코스를 중간에 그만둘 때, 해당 API를 한 번 요청해 주세요.")
    void quit(@MemberId final Long memberId, @PathVariable("id") @NotNull(message = "[ERROR] id 값이 존재하지 않습니다.") final Long randomId);

    @PostMapping("/{id}/end")
    @Operation(summary = "랜덤 대화 주제 코스 종료 API", description = "랜덤 대화 주제 코스 종료 API 입니다. 랜덤 대화 주제 코스를 종료할 때, 해당 API를 한 번 요청해 주세요.")
    void end(@MemberId final Long memberId, @PathVariable("id") @NotNull(message = "[ERROR] id 값이 존재하지 않습니다.") final Long randomId);

    @PostMapping("/{id}/record")
    @Operation(summary = "랜덤 대화 주제 코스 기록 API", description = "랜덤 대화 주제 코스 기록 API 입니다. 톡픽 선택할 때마다 기록을 위해, 해당 API를 한 번 요청해 주세요.")
    void record(@MemberId final Long memberId, @PathVariable("id") @NotNull(message = "[ERROR] id 값이 존재하지 않습니다.") final Long randomId, @RequestBody RandomReqDTO.Record requestDTO);

    @PostMapping("/{id}/rate")
    @Operation(summary = "랜덤 대화 주제 코스 평점 저장 API", description = "랜덤 대화 주제 코스 평점 저장 API 입니다.")
    void rate(@MemberId final Long memberId, @PathVariable("id") @NotNull(message = "[ERROR] id 값이 존재하지 않습니다.") final Long randomId, @RequestBody RandomReqDTO.Rate requestDTO);

    @PostMapping("/{id}/comment")
    @Operation(summary = "랜덤 대화 주제 코스 한 줄 평 저장 API", description = "랜덤 대화 주제 코스 한 줄 평 저장 API 입니다.")
    void comment(@MemberId final Long memberId, @PathVariable("id") @NotNull(message = "[ERROR] id 값이 존재하지 않습니다.") final Long randomId, @RequestBody RandomReqDTO.Comment requestDTO);
}